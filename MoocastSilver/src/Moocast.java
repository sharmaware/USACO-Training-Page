...
public class Moocast {
    ...
    public static void maxCows(int [][] cows ){
        for(int i = 0; i < cows[1].length; i++){
            for(int j = 0; j < cows[1].length; j ++){
                if(cows[3][j] == 0){
                    if(cows[3][i] >= Math.sqrt( Math.pow(cows[0][j] - cows[0][i], 2) + Math.pow(cows[1][j] - cows[1][i], 2))){
                      cows[3][i] = 1;
                      maxCows(cows);
                    }
                    else{
                        int cowsReached = 0;
                        for(int b = 0; b < cows[3].length; b++){
                            if(cows[3][b] == 1){
                                cowsReached += 1;
                            }
                            numCows.add(cowsReached);
                        }
                    }
                }
                /*boolean allReached = true;
                for(int a = 0; a < cows[3].length; a++){
                    if(cows[3][i] != 0){
                        allReached = false;
                    }
                }
                ...
                if(allReached){
                    numCows.add(cows[0].length);
                }
                */
                //if allReached add cows[0].length to cows
            }
        }
    }
